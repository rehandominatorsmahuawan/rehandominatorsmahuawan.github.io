...
window.editStats=id=>{const s=(typeof XD!=='undefined'?XD.stats:[]).find(z=>z.playerId===id)||{};modal('EDIT PLAYER STATS',[['matches','MATCHES'],['runs','RUNS'],['wickets','WICKETS'],['highScore','HIGHEST SCORE'],['fours','FOURS'],['sixes','SIXES'],['fifties','50s'],['hundreds','100s'],['runOuts','RUN OUTS'],['strikeRate','STRIKE RATE'],['economy','ECONOMY']].map(([name,label])=>({name,label,type:'number',value:s[name]||0})).concat([{name:'bestBowling',label:'BEST BOWLING',value:s.bestBowling||''},{name:'last5',label:'LAST 5 MATCH FORM',value:s.last5||''},{name:'season',label:'SEASON / YEAR',value:s.season||'2027'}]),async d=>{await rdmDB.collection('PlayerStats').doc(id).set({...d,playerId:id,matches:N(d.matches),runs:N(d.runs),wickets:N(d.wickets),highScore:N(d.highScore),fours:N(d.fours),sixes:N(d.sixes),fifties:N(d.fifties),hundreds:N(d.hundreds),runOuts:N(d.runOuts),strikeRate:N(d.strikeRate),economy:N(d.economy),updatedAt:rdmStamp()},{merge:true});await xload()})};
